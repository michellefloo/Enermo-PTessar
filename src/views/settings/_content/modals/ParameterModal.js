...
const ParameterUpdateModal = () => {
  ...
  const mutationUpdateParameter = useUpdateParameter((oldData, newData) => [
    ...oldData,
    newData,
  ]);
  ...
  const ParamterModalForm = forwardRef((_, ref) => {
    const formik = useFormik({
      ...
      onSubmit: async (values) => {
        ...
        try {
          ...
          const promise4 = await mutationUpdateParameter.mutateAsync({
            id_field_budget: 10004,
            budget_limit: values.price_per_kwh.toString(),
          });
    ...
    return (
      <CForm>
        ...
        <CFormGroup row>
          <CCol md="4" xs="4">
            <CLabel htmlFor="text-input">Price per KwH</CLabel>
          </CCol>
          <CCol xs="8" md="8">
            <CInput
              type="number"
              ...
              valid={!errors.price_per_kwh && !!values.price_per_kwh}
              invalid={touched.price_per_kwh && !!errors.price_per_kwh}
              onChange={handleChange}
              onBlur={handleBlur}
