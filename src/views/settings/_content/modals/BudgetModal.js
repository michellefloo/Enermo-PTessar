...
const BudgetUpdateModal = () => {
    ...
    const mutationUpdateBudget = useUpdateBudget((oldData, newData) => [...oldData, newData])
            ...
            onSubmit: async (values) => {
                ...
                try {
                    ...
                    Object.keys(values).forEach(key => {
                        mutateAsyncArr.push(mutationUpdateBudget.mutateAsync({
                            id_field_budget: key,
                            budget_limit: values[key].toString()
                        }))
                    })
                    Promise.all(mutateAsyncArr)
                    .then((res) => {
                        if(res[0].status === 200){
            ...
            <CForm>
                ...
                {MONTHS_ARRAY.map((month, index) => (
                        <CFormGroup row key={index}>
                            <CCol md="4" xs="4">
                                <CLabel htmlFor="text-input">Budget Limit for {month} {currentYear}</CLabel>
                            </CCol>
                            <CCol xs="8" md="8">
                            <CInput 
                                type="number"
                                ...
                                valid={!errors[index+1+currentYear] && !!values[index+1+currentYear]}
                                invalid={touched[index+1+currentYear] && !!errors[index+1+currentYear]}
                                onChange={handleChange}
                                onBlur={handleBlur}
