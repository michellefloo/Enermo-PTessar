...
const GetKwhAvg = (id_device, sensor) => {
    const { data, status } = useGetKwhAvg(id_device, sensor.id, 
...
const RealKwhThisMonth = () => {
    ...
    if(allRqStatus === "loading"){
        data.result.map((device, index) => {
            if(device.is_main_panel === 1){
                kwhAvgArr.push(GetKwhAvg(device.id, 
                    sensorData.result.find(s => s.id_device === device.id && s.sensor_type === 'edel' ))
                )
            }
        })
    }
    ...
    if((counter >= kwhAvgArr.length))
        return (
            <Fragment>
                <CWidgetIcon color="primary" iconPadding={false}
                            header={getAllTotal().toLocaleString()} text="REAL KWH THIS MONTH" 
