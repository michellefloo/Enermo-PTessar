...
import { useAddUtility, useGetUtilityTypeList,
...
const UtilityAddSchema = Yup.object().shape({
...
const UtilityAddModal = () => {
    ...
    const mutationAddUtility = useAddUtility((oldData, newData) => [...oldData, newData])

    useEffect(() => {  
        setToggleModal(modalName === "addUtilityModal")
    }, [modalName])

    const UserModalForm = forwardRef((_, ref) => {
        const formik = useFormik({
            initialValues: utilityData? utilityData : initialValues,
            validationSchema: UtilityAddSchema,
            onSubmit: async (values) => {
                ...
                try {
                    ...
                    await mutationAddUtility.mutateAsync(values)
                    .then((res) => {
                        if(res.status === 200){
...
export default UtilityAddModal;