...
import { useAddSensor, useGetSensorTypeSelectList, useGetSensorClassSelectList} from 'src/api/sensor'
...
const SensorAddSchema = Yup.object().shape({
...
const SensorAddModal = () => {
    ...
    const mutationAddUser = useAddSensor((oldData, newData) => [...oldData, newData])

    useEffect(() => {  
        setToggleModal(modalName === "addSensorModal")
    }, [modalName])

    const UserModalForm = forwardRef((_, ref) => {
        const formik = useFormik({
            initialValues: sensorData? sensorData : initialValues,
            validationSchema: SensorAddSchema,
