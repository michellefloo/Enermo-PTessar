...
import { useAddDevice} from 'src/api/device'
import { useGetFabricatorSelectList } from "src/api/fabricator";
import { toast } from 'react-toastify';
import * as Yup from 'yup';

const DeviceAddSchema = Yup.object().shape({
...
const DeviceAddModal = () => {
    ...
    const mutationAddUser = useAddDevice((oldData, newData) => [...oldData, newData])

    useEffect(() => {  
        setToggleModal(modalName === "addDeviceModal")
    }, [modalName])

    const UserModalForm = forwardRef((_, ref) => {
        const formik = useFormik({
            initialValues: deviceData? deviceData : initialValues,
            validationSchema: DeviceAddSchema,
