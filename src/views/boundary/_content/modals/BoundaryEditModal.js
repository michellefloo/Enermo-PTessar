...
import {
  useUpdateBoundary,
  useGetBoundarySelectList,
  useGetBoundaryTypeList,
} from "../../../../api/boundary";
import { useGetCustomerSelectList } from "src/api/customer";
import { toast } from "react-toastify";
import * as Yup from "yup";

const BoundaryEditSchema = Yup.object().shape({
  location_code: Yup.string().required("Boundary code is required"),

  location_name: Yup.string().required("Boundary name is required"),
  ...
  location_square_meter: Yup.number().required(
    "Boundary square meter is required"
  ),
});

const BoundaryEditModal = () => {
  ...
  const { data: boundarySelectList, status: boundarySelectListStatus } =
    useGetBoundarySelectList();
  const { data: customerSelectList, status: customerSelectListStatus } =
    useGetCustomerSelectList();
  const { data: boundaryTypeList, status: boundaryTypeListStatus } =
    useGetBoundaryTypeList();
  const mutationUpdateBoundary = useUpdateBoundary((oldData, newData) => [
    ...oldData,
    newData,
  ]);

  useEffect(() => {
    setToggleModal(modalName === "editBoundaryModal");
  }, [modalName]);

  const UserModalForm = forwardRef((_, ref) => {
    const formik = useFormik({
      initialValues: initialValues,
      validationSchema: BoundaryEditSchema,
      onSubmit: async (values) => {
        setSubmitting(true);
        try {
          values.idParents =
            values.idParents.value === -1 ? [null] : [values.idParents.value];
          values.id_location_type = values.id_location_type.value;
          values.id_main_customer = values.id_main_customer.value;
          if (values.id_main_customer === -1) values.id_main_customer = null;
          delete values.customer;
          delete values.customer_name;
          await mutationUpdateBoundary.mutateAsync(values).then((res) => {
            if (res.status === 200) {
        ...
        <CFormGroup row>
          ...
          <CCol xs="8" md="8">
            <Select
              ...
              options={
                boundarySelectList?.filter(
                  (b) => b.value !== initialValues.id
                ) || []
              }
        ...
        <CFormGroup row>
          ...
          <CCol xs="8" md="8">
            <CInput
              ...
              valid={
                !errors.location_square_meter && !!values.location_square_meter
              }
              invalid={
                touched.location_square_meter && !!errors.location_square_meter
              }
              onChange={handleChange}
              onBlur={handleBlur}
...
export default BoundaryEditModal;
