...
import { useUpdateBoundary, useGetBoundarySelectList, useGetBoundaryTypeList } from '../../../../api/boundary'
...
const BoundaryEditModal = () => {
    ...
    const mutationUpdateBoundary = useUpdateBoundary((oldData, newData) => [...oldData, newData])
            ...
            onSubmit: async (values) => {
                ...
                try {
                    ...
                    if(values.id_main_customer === -1)
                        values.id_main_customer = null
                    delete values.customer
                    delete values.customer_name
                    await mutationUpdateBoundary.mutateAsync(values)
                    .then((res) => {
                        if(res.status === 200){
                ...
                <CFormGroup row>
                    ...
                    <CCol xs="8" md="8">
                        <Select
                            ...
                            options={boundarySelectList?.filter(b => b.value !== initialValues.id) || []}
