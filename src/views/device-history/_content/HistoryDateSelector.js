...
const HistoryDateSelector = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      if (
        moment(values.end_date).diff(moment(values.start_date), "days") >= 5
      ) {
  ...
  return (
    <CCard>
      <CCardHeader>History Date Selector</CCardHeader>
      <CCardBody>
        <CForm>
          <CFormGroup row>
            <CCol md="4" xs="4">
              <CLabel htmlFor="text-input">Start Date</CLabel>
            </CCol>
              ...
              <Datetime
                ...
                onChange={(value) => {
                  const date = moment(value, "YYYY-MM-DD", true).isValid()
                    ? value.format(DATE_ISO8601_NO_TIME)
                    : value;
