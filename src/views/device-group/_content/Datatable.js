...
const GroupInstallationDatatable = () => {
  ...
  const { data: rqData, status: rqStatus } =
    useGetDeviceGroupDetailListByIdGroup(selectedGroup);
  ...
  useEffect(() => {
    ...
    if (rqStatus === "success" && deviceStatus === "success") {
      ...
      rqData.result.map((item1) => {
        const matchingItem = deviceData.result.find(
          (item2) => item2.id === item1.id_devices
        );
        if (matchingItem) {
