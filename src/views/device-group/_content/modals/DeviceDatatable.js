...
const ActionButtonSlot = (props) => {
  // const {toggleSensorDetails, toggleInstallationDetails,
  //         toggleAddInstallation, onUpdateClick, detailType,
  //         onDeleteClick, data, details} = props
  return (
    <td>
      <CDropdown>
        ...
        <CDropdownMenu>
          ...
          {/* <CDropdownItem onClick={() => toggleSensorDetails(data)}>
                        <CRow>
                            <CCol>{(details.id === data.id && detailType === 'sensor')? "Hide" : "Show"} Sensors</CCol>
                            <CCol className="ml-5">
                    ...
                    <CDropdownItem onClick={() => toggleInstallationDetails(data)}>
                        <CRow>
                        <CCol>{(details.id === data.id && detailType === 'installation')? "Hide" : "Show"} Installations</CCol>                            
                        <CCol className="ml-5">
...
//Better using foward reff on this and imperative handler but for now this is fine
const DeviceDatatable = (props) => {
  ...
  useEffect(() => {
    if (rqStatus === "success") {
      ...
      //    const deviceDataArr = []
      //    const tempRqData = [...rqData.result]
      //    tempRqData.map(async item1 => {
      //     const alreadySelected = await props.deviceGroupDetailData.find(item2 => item2.id_devices === item1.id);
      //     if (!alreadySelected) {
      //         item1._checked = true
      //         deviceDataArr.push(item1)
      //     }
      //     deviceDataArr.push(item1)
      //     return
      //   });
      if (deviceData.length === 0) setDeviceData(rqData.result);
    }
  }, [rqData, rqStatus]);

  useEffect(() => {
    props.onDeviceSelect(deviceData);
  }, [deviceData]);
