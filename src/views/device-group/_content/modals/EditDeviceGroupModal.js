...
} from "src/api/device-group";
import {
  useAddDeviceGroupDetail,
  useGetDeviceGroupDetailList,
} from "src/api/device-group-detail";
...
const DeviceGroupEditModal = () => {
  ...
  const { data: deviceGroupDetailData, status: deviceGroupDetailStatus } =
    useGetDeviceGroupDetailList();
  const mutationEditDeviceGroup = useUpdateDeviceGroup((oldData, newData) => [
    ...oldData,
    newData,
  ]);
  // const mutationEditDeviceGroupDetail = useUpdateDeviceGroupDetail((oldData, newData) => [...oldData, newData], initialValues.id_group)
  const mutationAddDeviceGroupDetail = useAddDeviceGroupDetail(
  ...
  const handleOnDeviceSelect = (newState) => {
    ...
    if (isEqual(newState, selectedDeviceData)) return;
    selectedDeviceData = newState; //LOL :D as long as it works
  };

  const submitDeviceToGroupFactory = async (device) => {
    const deviceExistInGroup = deviceUponShow.find(
      (d) => d.id_devices === device.id
    );
  ...
  const UserModalForm = forwardRef((_, ref) => {
    const formik = useFormik({
      initialValues: deviceInstallationData
                ...
                await submitDeviceToGroupFactory(d)
                  .then((res) => {
                    ...
                    if (res.data.status === 200) {
                      toast.success(
                        "Device " + d.device_desc + " updated from/to Group"
                      );
                    }
                  })
                  .catch((e) => {
