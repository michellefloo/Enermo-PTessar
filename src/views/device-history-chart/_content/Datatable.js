...
import { CycleDatatable } from "./sensor-product/index";
...
const HistoryDeviceDatatable = () => {
  ...
  const { selectedStartDate, selectedEndDate, selectedDevice } = useContext(
    DeviceHistoryContext2
  );
  const [historyResult, setHistoryResult] = useState(null);
  const { data: rqDataMonitoring, status: rqStatusMonitoring } =
    useGetDeviceMonitoringByDate(
      selectedDevice.id,
      moment(selectedStartDate).unix(),
      moment(selectedEndDate).unix()
    );

  useEffect(async () => {
    async function getHistoryResult() {
      if (rqStatusMonitoring === "success" && !historyResult)
  ...
  useEffect(() => {
    ...
    dispatch({
      ...
      data: cycleDetailArr,
      fileName: `Cycle Detail of ${selectedDevice.name} period ${moment(
        selectedStartDate
      ).format(DATE_ISO8601_NO_TIME)} - ${moment(selectedEndDate).format(
        DATE_ISO8601_NO_TIME
      )}`,
    });
