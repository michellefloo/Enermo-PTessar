...
import { useUpdateCustomer } from "src/api/customer";
...
const CustomerEditSchema = Yup.object().shape({
  ...
  main_address: Yup.string().required("Main Editress is required"),
...
const CustomerEditModal = () => {
  ...
  const mutationEditUser = useUpdateCustomer((oldData, newData) => [
    ...oldData,
    newData,
  ]);

  useEffect(() => {
    setToggleModal(modalName === "editCustomerModal");
  }, [modalName]);
  ...
  const UserModalForm = forwardRef((_, ref) => {
    const formik = useFormik({
      ...
      onSubmit: async (values) => {
        ...
        try {
          await mutationEditUser.mutateAsync(values).then((res) => {
            if (res.status === 200) {
        ...
        <CFormGroup row>
          ...
          <CCol xs="8" md="8">
            <CTextarea
              ...
              valid={!errors.description && !!values.description}
              ...
              disabled={isSubmitting}
            />
            <CFormText color="danger">{errors.description}</CFormText>
          </CCol>
        </CFormGroup>
      </CForm>
    );
